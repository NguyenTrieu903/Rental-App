...
import { httpResponses } from "../utils/responseHandler";
import { APPLICATION_STATUS } from "../utils/contants";
...
export const createApplication = async (
  ...
  try {
    ...
    httpResponses.created(res, newApplication);
  } catch (error: any) {
    httpResponses.serverError(
      res,
      `Error retrieving application: ${error.message}`
    );
    // res
    //   .status(500)
    //   .json({ message: `Error retrieving application: ${error.message}` });
...
export const updateApplicationStatus = async (
  ...
  try {
    ...
    if (!application) {
      ...
      httpResponses.notFound(res, "Application not found");
      return;
    }
    ...
    httpResponses.success(res, updatedApplication);
  } catch (error: any) {
    httpResponses.serverError(
      res,
      `Error updatring application status: ${error.message}`
    );
    // res
    //   .status(500)
    //   .json({ message: `Error retrieving application: ${error.message}` });
